...
static void permute(unsigned char *out, const unsigned char *in, const unsigned char *p, unsigned n)
{
	unsigned i;
	for (i=0;i<n;i++)
		out[i] = in[p[i]-1];
...
static void lshift(unsigned char *d, unsigned count, unsigned n)
{
...
static void concat(unsigned char *out, const unsigned char *in1, const unsigned char *in2, unsigned l1, unsigned l2)
{
	while (l1--)
		*out++ = *in1++;
	while (l2--)
...
static void xor(unsigned char *out, const unsigned char *in1, const unsigned char *in2, unsigned n)
{
	unsigned i;
	for (i=0;i<n;i++)
		out[i] = (unsigned char)(in1[i] ^ in2[i]);
}

static void dohash(unsigned char *out, const unsigned char *in, const unsigned char *key, int forw)
{
	int i, j, k;
	unsigned char pk1[56];
	unsigned char c[28];
	unsigned char d[28];
	unsigned char cd[56];
	unsigned char ki[16][48];
	unsigned char pd1[64];
	...
	for (i=0;i<16;i++) {
		unsigned char er[48];
		unsigned char erk[48];
		unsigned char b[8][6];
		unsigned char cb[32];
		unsigned char pcb[32];
		unsigned char r2[32];
		...
		for (j=0;j<8;j++) {
			...
			for (k=0;k<4;k++)
				b[j][k] = (unsigned char)((sbox[j][m][n] & (1<<(3-k)))?1:0);
		}
...
void crypt_des_ecb(unsigned char *out, const unsigned char *in, const unsigned char *key, unsigned forw)
{
	int i;
	unsigned char outb[64];
	unsigned char inb[64];
	unsigned char keyb[64];
	unsigned char key2[8];

	str_to_key(key, key2);

	for (i=0;i<64;i++) {
		inb[i] = (unsigned char)((in[i/8] & (1<<(7-(i%8)))) ? 1 : 0);
		keyb[i] = (unsigned char)((key2[i/8] & (1<<(7-(i%8)))) ? 1 : 0);
		outb[i] = 0;
	}
